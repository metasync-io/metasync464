...
public:
	...
	uint32_t GetRegionX() const { return (x >> 3); }

	uint32_t GetRegionY() const { return (y >> 3); }

	uint32_t GetLocalX(const Position& base) const { return x - 8 * (base.GetRegionX() - 6); }

	uint32_t GetLocalY(const Position& base) const { return y - 8 * (base.GetRegionY() - 6); }
	...
	bool IsWithinInteractionDistance(const Position& other) const {
