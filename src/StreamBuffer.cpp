...
std::string StreamBuffer::ReadString()
{
	...
	size_t count = 0;
	while ((byte = static_cast<uint8_t>(ReadByte(false))) != 0)
	{

		result += static_cast<char>(byte);
		//LOG_INFO("[StreamBuffer::ReadString()] Count = {}, Result: {}", count, result);
...
void StreamBuffer::WriteTriByte(int32_t value)
{
	WriteByte(value >> 16);
	WriteByte(value >> 8);
	WriteByte(value);
...
void StreamBuffer::WriteSmart(int32_t value)
{
	if (value >= 128) {
...
void StreamBuffer::WriteSignedSmart(int32_t value)
{
	if (value >= 128) {
...
void StreamBuffer::WriteReverse(const int8_t* data, int32_t offset, int32_t length)
{
...
void StreamBuffer::WriteReverseA(const int8_t* data, int32_t offset, int32_t length)
{
	for (int32_t i = (offset + length - 1); i >= offset; i--) {
		WriteByte(data[i], ValueType::A);
	}
}

void StreamBuffer::Skip(int32_t count)
{
	if (m_Pos + count > m_Buffer.size()) {
		m_Buffer.resize(m_Pos + count);
	}
	m_Pos += count;
}
...
void StreamBuffer::WriteBits(int32_t amount, int32_t value)
{
	...
	if (neededBytes > static_cast<int32_t>(m_Buffer.size())) {
		m_Buffer.resize(neededBytes + DEFAULT_BUFFER_SIZE);
	}
