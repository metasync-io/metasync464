...
namespace Skeleton {
    ...
    std::string TextUtils::DecryptPlayerChat(StreamBuffer& buffer, int32_t totalChars) {
        try {
            if (totalChars == 0) {
                return "";
            }

            int32_t charsDecoded = 0;
            int32_t i_4 = 0;
            std::string s = "";

            for (;;) {
                int8_t i_7 = buffer.ReadByte();

                if (i_7 >= 0) {
                    i_4++;
                } else {
                    i_4 = CHAT_DECRYPT_KEYS[i_4];
                }

                int32_t i_8;
                if ((i_8 = CHAT_DECRYPT_KEYS[i_4]) < 0) {
                    s += (char)(int8_t)(i_8 ^ 0xffffffff);
    ...
    std::string TextUtils::FilterText(const std::string& text) {
        static const std::string VALID_CHARS = " abcdefghijklmnopqrstuvwxyz0123456789!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";

        std::string result;
        for (char c : text) {
            char lower = std::tolower(c);
            if (VALID_CHARS.find(lower) != std::string::npos) {
                result += lower;
            }
        }
        return result;
    ...
    std::string TextUtils::OptimizeText(const std::string& text) {
    ...
    void TextUtils::TextPack(uint8_t* packedData, const std::string& text) {
        std::string workText = text;
        if (workText.length() > 80) {
            workText = workText.substr(0, 80);
        }

        int32_t length = workText.length();
        int32_t key = 0;
        int32_t bitPosition = 1 << 3;
        int32_t srcOffset = 0;

        packedData[0] = (uint8_t)length;

        for (; srcOffset < length; srcOffset++) {
            int32_t textByte = (uint8_t)workText[srcOffset] & 0xff;
            int32_t mask = CHAT_MASKS[textByte];
            int32_t size = CHAT_BIT_SIZES[textByte];
            int32_t destOffset = bitPosition >> 3;
            int32_t bitOffset = bitPosition & 0x7;

            key &= (-bitOffset >> 31);
            bitPosition += size;
            int32_t byteSize = (((bitOffset + size) - 1) >> 3) + destOffset;
            bitOffset += 24;
            packedData[destOffset] = (uint8_t)(key = (key | ((uint32_t)mask >> bitOffset)));

            if (byteSize > destOffset) {
                destOffset++;
                bitOffset -= 8;
                packedData[destOffset] = (uint8_t)(key = (uint32_t)mask >> bitOffset);
                if (byteSize > destOffset) {
                    destOffset++;
                    bitOffset -= 8;
                    packedData[destOffset] = (uint8_t)(key = (uint32_t)mask >> bitOffset);
                    if (byteSize > destOffset) {
                        ...
                        if (destOffset < byteSize) {
                            bitOffset -= 8;
                            destOffset++;
                            packedData[destOffset] = (uint8_t)(key = mask << -bitOffset);
                        }
                    }
                }
            }
